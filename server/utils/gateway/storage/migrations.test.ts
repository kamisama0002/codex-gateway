...
import { MANAGED_RUNTIME_HOST_ID, migrateGatewayDatabase } from "./migrations";
...
describe("migrateGatewayDatabase", () => {
  it("adds user roles and the per-user runtime table idempotently", () => {
    ...
    db.prepare(
      ...
      `,
    ).run(1, "codex-app-server", "image-1", "runtime-1", "schema-1", "absent", "2024-01-01T00:00:00.000Z", "2024-01-01T00:00:00.000Z");
    expect(db.prepare("SELECT host_id FROM user_agent_runtimes WHERE user_id = ?").get(1)).toMatchObject({
