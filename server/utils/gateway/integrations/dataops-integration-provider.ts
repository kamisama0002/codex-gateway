import { createDataOpsSsoClient, DataOpsSsoError, type DataOpsSsoClient } from "../auth/dataops-client";
...
type IntegrationRepository = Pick<ReturnType<typeof createDataOpsIntegrationRepository>, "acceptedForAuthentication">;
...
export function createDataOpsIntegrationProvider(options: {
  ...
  return {
    async current(): Promise<DataOpsIntegrationSnapshot | null> {
      ...
      const snapshot = bindings.length === 0 ? null : {
        ...
        client: fallbackClient(bindings.map((binding) => createClient({
    ...
    invalidate(revision?: number): void {
      if (cached === null || revision === undefined || cached.snapshot === null || revision >= cached.snapshot.revision) {
        cached = null;
      }
    },
  };
