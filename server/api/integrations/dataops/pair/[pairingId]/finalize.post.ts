...
export async function finalizeDataOpsPairingForEvent(
  event: H3Event,
  pairingId = getRouterParam(event, "pairingId") ?? "",
  service: Pick<typeof dataOpsPairingService, "finalize"> = dataOpsPairingService,
) {
  ...
  return await service.finalize(pairingId, body.revision, bearerSecret);
}

export default defineGatewayEventHandler((event) => finalizeDataOpsPairingForEvent(event));
